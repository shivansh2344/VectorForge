...
// Doc embeddings dimension is determined at runtime from the configured service model output
...
// =====================================================================
//  SERVICE CLIENT  — wraps an external embedding/generation HTTP API
//  Configure an external embedding/generation service if you require document embeddings.
// =====================================================================
...
class ServiceClient {
    ...
    static bool parseHostPort(const std::string& raw, std::string& outHost, int& outPort) {
        ...
        if (colon != std::string::npos && colon + 1 < s.size()) {
            outHost = s.substr(0, colon);
            try { outPort = std::stoi(s.substr(colon + 1)); } catch (...) { return false; }
        } else {
            outHost = s;
        }
    ...
    // Parse {"embedding":[...]} from embedding service /api/embeddings response
    ...
    // Parse {"embeddings":[[...]]} from newer /api/embed response
    std::vector<float> parseEmbeddingsV2(const std::string& body) {
    ...
    // Parse {"response":"..."} from generation service /api/generate response
...
public:
    ...
    std::string genModel   = "llama3.2:3b";

    ServiceClient(const std::string& h = "127.0.0.1", int p = 11434)
        : host(h), port(p) {
        // Allow runtime overrides without recompiling.
        std::string cfgHost = envOr("SERVICE_HOST", "");
        if (cfgHost.empty()) cfgHost = envOr("OLLAMA_HOST", "");
        if (!cfgHost.empty()) parseHostPort(cfgHost, host, port);

        std::string cfgPort = envOr("SERVICE_PORT", "");
        if (!cfgPort.empty()) {
            try { port = std::stoi(cfgPort); } catch (...) {}
        }

        embedModel = envOr("EMBED_MODEL", embedModel);
        embedModel = envOr("OLLAMA_EMBED_MODEL", embedModel);
        genModel   = envOr("GEN_MODEL", genModel);
        genModel   = envOr("OLLAMA_GEN_MODEL", genModel);
    }
    ...
    // Returns empty vector if the service is not reachable or model not found
    std::vector<float> embed(const std::string& text) {
        httplib::Client cli(host, port);
        cli.set_connection_timeout(3, 0);
        cli.set_read_timeout(30, 0);
        std::string bodyV1 = "{\"model\":\"" + embedModel + "\",\"prompt\":\"" + esc(text) + "\"}";
        auto res = cli.Post("/api/embeddings", bodyV1, "application/json");
        if (res && res->status == 200) {
            auto v = parseEmbedding(res->body);
            if (!v.empty()) return v;
        }

        std::string bodyV2 = "{\"model\":\"" + embedModel + "\",\"input\":\"" + esc(text) + "\"}";
        res = cli.Post("/api/embed", bodyV2, "application/json");
        if (res && res->status == 200) {
            auto v = parseEmbeddingsV2(res->body);
            if (!v.empty()) return v;
    ...
    // Returns error string if the generation service is unavailable
    std::string generate(const std::string& prompt) {
        httplib::Client cli(host, port);
        cli.set_connection_timeout(3, 0);
        cli.set_read_timeout(180, 0);   // generation can be slow
        std::string body = "{\"model\":\"" + genModel + "\","
                           "\"prompt\":\"" + esc(prompt) + "\","
                           "\"stream\":false}";
        auto res = cli.Post("/api/generate", body, "application/json");
        if (!res || res->status != 200)
            return "ERROR: external generation service unavailable.";
        return parseResponse(res->body);
    }
};

// =====================================================================
//  DOCUMENT DATABASE  — HNSW over real document embeddings
// =====================================================================
...
int main() {
    ...
    // Check external service at startup (non-fatal)
    bool serviceUp = service.isAvailable();
    std::cout << "=== VectorForge — SHIVANSH GARG ===" << std::endl;
    std::cout << "http://localhost:8080" << std::endl;
    std::cout << db.size() << " demo vectors | " << DIMS << " dims | HNSW+KD-Tree+BruteForce" << std::endl;
    std::cout << "External Service: " << (serviceUp ? "ONLINE" : "OFFLINE (configure external service)") << std::endl;
    if (serviceUp) std::cout << "  embed model: " << service.embedModel
                            << "  gen model: "   << service.genModel << std::endl;
    ...
    // Chunks the text, optionally embeds each chunk via an external service, stores in DocumentDB
    svr.Post("/doc/insert", [&](const httplib::Request& req, httplib::Response& res) {
        ...
        for (int i = 0; i < (int)chunks.size(); i++) {
            auto emb = service.embed(chunks[i]);
            if (emb.empty()) {
                res.set_content(
                    "{\"error\":\"Embedding service unavailable. Configure an external embedding service and retry.\"}",
                    "application/json");
                return;
            }
    ...
    svr.Post("/doc/search", [&](const httplib::Request& req, httplib::Response& res) {
        ...
        auto qEmb = service.embed(question);
        if (qEmb.empty()) {
            res.set_content("{\"error\":\"Embedding service unavailable\"}", "application/json"); return;
        }
    ...
    svr.Post("/doc/ask", [&](const httplib::Request& req, httplib::Response& res) {
        ...
        auto answer = service.generate(prompt);

        // Step 5: return everything
        std::ostringstream ss;
          ss << "{\"answer\":" << jS(answer)
              << ",\"model\":"  << jS(service.genModel)
           << ",\"contexts\":[";
        for (size_t i = 0; i < hits.size(); i++) {
            if (i) ss << ',';
            ss << "{\"id\":"       << hits[i].second.id
    ...
    svr.Get("/status", [&](const httplib::Request&, httplib::Response& res) {
        cors(res);
          bool up = service.isAvailable();
          std::ostringstream ss;
          ss << "{\"serviceAvailable\":"  << (up ? "true" : "false")
              << ",\"embedModel\":"       << jS(service.embedModel)
              << ",\"genModel\":"         << jS(service.genModel)
           << ",\"docCount\":"         << docDB.size()
